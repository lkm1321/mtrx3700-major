...
$Source: C:\\RCS\\d\\salvo\\src\\qins.c,v $
$Author: aek $
$Revision: 3.7 $
    ...
    if ( !thisTcbP ) {
        *tcbPP = tcbP;
        ...
        /* find insertion point based on delay ... */
            ...
            /*  plify what would normally be written as:    */
            /*      thisTcbP->dly.delay -= tcbP->dly.delay; */
            /*  (was too complex for PIC-C).                */
            if ( thisTcbP )
                u.totDelay -= thisTcbP->dly.delay;
        ...
        if ( !prevTcbP )
            *tcbPP = tcbP;
            ...
            prevTcbP->nextTcbP = tcbP;
            
        tcbP->nextTcbP = thisTcbP;        
...
{ 
    ...
    if ( !thisTcbP ) {
        *tcbPP = tcbP;
        ...
        /*  inserting.                                    */
        while ( prio >= thisTcbP->status.bits.prio ) {    
            prevTcbP = thisTcbP;
            thisTcbP = thisTcbP->nextTcbP;
