...
$Source: C:\\RCS\\d\\salvo\\src\\inittask.c,v $
$Author: aek $
$Revision: 3.22 $
...
{    
    ...
    OSInitPrioTask(tcbP, (OStypePrio) (prio & ~OSDONT_START_TASK));
    #else
    tcbP->status.bits.prio = (OStypePrio) (prio & ~OSDONT_START_TASK);
    #endif
    ...
    #else
    tcbP->status.bits.prio = OSONLY_PRIO;
    #endif
    ...
    OSMsgRtn("OSCreateTask", 
      OSMakeStr("task %d created.", OStID(tcbP)), 
      (OStypeErr) OSNOERR);
}
